...
def Torrents(url):
	for i in range(1,6):
		try:
			...
			return
		if not link: return
		match = re.compile('itemprop="headline"><a href="(https:..comandotorrents.org[^"]+)..([^\<]+)').findall(link)
		img = re.compile('class="alignleft(size-full)? .+?" src="([^"]+)').findall(link)
		i = 0
		for link,title in match:
			xx.AddDir(unquote(title), link, "comando.ListTorrents", img[i][1], img[i][1], isFolder=True)
			i+=1
def ListTorrents():
		#xx.AddDir(str(i), "", "comando.ListTorrents", "", isFolder=True)
	...
	for title,espaco,link in match:
		...
		xx.AddDir(unquote(title2[0][0]), link, "comando.PlayTorrents", iconimage, logos, isFolder=False, IsPlayable=True)
def PlayTorrents():
